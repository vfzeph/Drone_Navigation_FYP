...
def configure_logger(name, log_dir='./drone/logs'):
...
def train_agent(agent, env, config, logger, writer, scheduler, data_processor, data_visualizer):
    ...
    model_save_path = os.path.join(current_dir, 'drone/models/checkpoint')
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        while not done:
            ...
            # Ensure losses and entropy are initialized
        ...
        # Read and process TensorBoard logs
...
def main():
    config_path = os.path.join(current_dir, 'configs', 'learning', 'ppo_config.json')
